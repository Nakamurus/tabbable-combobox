...
export const Combobox2: React.FC<ComboboxProps> = ({
  options,
  value,
  placeholder,
  onSelectionChange,
  ...
  // Use custom hook with querySelectorAll for focus management (Approach 2 requirement)
  const { getNextTabbable, getPreviousTabbable } = useFocusManagement();
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case KEYS.ESCAPE:
        ...
        break;
      case KEYS.TAB: {
        // Close popup and use custom focus management with form scoping (Approach 2)
        if (isOpen) {
          setIsOpen(false);
          setHighlightedIndex(0);
        }
        
        // Use querySelectorAll-based focus management with form scoping
        const currentElement = inputRef.current;
        if (currentElement) {
          const nextElement = e.shiftKey 
            ? getPreviousTabbable(currentElement)
            : getNextTabbable(currentElement);
